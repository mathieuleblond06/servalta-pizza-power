      ...
      <div className="container-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial, index) => (
            <div 
              key={testimonial.id} 
              className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 animate-fade-in h-full flex flex-col"
              style={{ animationDelay: `${0.3 + index * 0.1}s` }}
            >
              <svg className="w-10 h-10 text-servalta-red/20 mb-4" fill="currentColor" viewBox="0 0 24 24">
